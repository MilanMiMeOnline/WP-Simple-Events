...
/** Renders linked event tags. */
final class EventTagsWidget extends AbstractEventFieldWidget {
	/** Return the stable widget identifier. */
	public function get_name(): string {
		return 'wpse-event-tags';
	}

	/** Return the translated editor title. */
	public function get_title(): string {
		return esc_html__( 'Event Tags', 'mime-simple-events-calendar' );
	}
	...
	public function get_keywords(): array {
		return array( 'event', 'tags', 'taxonomy' );
	}

	/** Register tag-label controls. */
	protected function register_field_controls(): void {
		...
		$this->add_label_controls( esc_html__( 'Tags:', 'mime-simple-events-calendar' ) );
	...
	protected function render_field( EventPresentation $presentation, array $settings ): string {
		return $this->fields->tags( $presentation, $this->show_label( $settings ), $this->label( $settings ) );
	}

	/** Return the tag style selector. */
