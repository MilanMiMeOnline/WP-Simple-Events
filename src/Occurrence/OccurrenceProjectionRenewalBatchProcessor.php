<?php
/**
 * Bounded recurring projection renewal.
 *
...
/**
 * Rebuilds only recurring public projections that crossed the renewal buffer.
 */
final readonly class OccurrenceProjectionRenewalBatchProcessor {
	public const BATCH_SIZE = 25;

	/**
	 * Create the bounded renewal processor.
	 *
	 * @param OccurrenceIndexRepairService $repairer Type-aware repair service.
	 * @param OccurrenceCoverageMetaQuery  $queries  Renewal candidate query builder.
	 */
	...
	/**
	 * Process one bounded candidate page after already unresolved candidates.
	 *
	...
	public function process( int $unresolved_offset = 0 ): OccurrenceIndexBatchResult {
		...
		try {
			$meta_query = $this->queries->renewal_due( $today );
		...
		$ids = get_posts(
			array(
				...
				'meta_query'             => $meta_query, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Fixed-size background maintenance over plugin-owned single-value metadata.
			)
		);
