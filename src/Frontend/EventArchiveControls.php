...
final class EventArchiveControls {
	...
	public function filters( EventListAttributes $attributes ): string {
		...
		ob_start();
		?>
		<form class="wpse-events-filters wpse-event-archive-filters" method="get" action="<?php echo esc_url( is_string( $action ) ? $action : '' ); ?>" aria-label="<?php esc_attr_e( 'Filter events', 'wp-simple-events' ); ?>">
			<p class="wpse-events-filter-field">
				<label for="wpse-archive-period"><?php esc_html_e( 'Period', 'wp-simple-events' ); ?></label>
			...
			<?php if ( array() !== $categories ) : ?>
				<?php $this->term_select( $categories, 'wpse_category', 'wpse-archive-category', __( 'Categories', 'wp-simple-events' ), $attributes->category_slugs ); ?>
			<?php endif; ?>

			<?php if ( array() !== $tags ) : ?>
				<?php $this->term_select( $tags, 'wpse_tag', 'wpse-archive-tag', __( 'Tags', 'wp-simple-events' ), $attributes->tag_slugs ); ?>
			<?php endif; ?>

			<p class="wpse-events-filter-submit">
				<button type="submit" aria-controls="wpse-archive-results"><?php esc_html_e( 'Apply filters', 'wp-simple-events' ); ?></button>
			</p>
		</form>
		<?php
		$output = ob_get_clean();
