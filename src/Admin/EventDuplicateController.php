...
final class EventDuplicateController {
	...
	public function row_actions( array $actions, WP_Post $post ): array {
		...
		$actions['wpse_duplicate'] = sprintf(
			'<a href="%1$s">%2$s</a>',
			esc_url( $url ),
			esc_html__( 'Duplicate event', 'mime-simple-events-calendar' )
		);
	...
	public function duplicate(): void {
		...
		if ( ! $source instanceof WP_Post || ! $this->can_duplicate( $source ) ) {
			wp_die(
				esc_html__( 'You are not allowed to duplicate this event.', 'mime-simple-events-calendar' ),
				esc_html__( 'Event duplication denied', 'mime-simple-events-calendar' ),
				array( 'response' => 403 )
			);
		}
		...
		if ( is_wp_error( $new_id ) ) {
			wp_die(
				...
				esc_html__( 'Event duplication failed', 'mime-simple-events-calendar' ),
				array( 'response' => 500 )
			);
		}
		...
		if ( ! is_string( $edit_link ) || '' === $edit_link ) {
			wp_die(
				esc_html__( 'The event was copied, but its editor URL is unavailable.', 'mime-simple-events-calendar' ),
				esc_html__( 'Event duplication incomplete', 'mime-simple-events-calendar' ),
				array( 'response' => 500 )
			);
		}
	...
	public function render_notice(): void {
		...
		<div class="notice notice-success is-dismissible">
			<p><?php esc_html_e( 'Event duplicated as a draft. Review the copied dates before publishing; the external event link and its label were not copied.', 'mime-simple-events-calendar' ); ?></p>
		</div>
		<?php
	}
