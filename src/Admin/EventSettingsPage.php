...
final class EventSettingsPage {
	...
	public function register_menu(): void {
		add_submenu_page(
			...
			__( 'Event settings', 'mime-simple-events-calendar' ),
			__( 'Settings', 'mime-simple-events-calendar' ),
			'manage_options',
			self::PAGE_SLUG,
	...
	public function register_settings(): void {
		...
		add_settings_section(
			...
			__( 'Event archive', 'mime-simple-events-calendar' ),
		...
		add_settings_field(
			EventArchiveSettings::SLUG_OPTION,
			__( 'Archive slug', 'mime-simple-events-calendar' ),
		...
		add_settings_field(
			EventArchiveSettings::PER_PAGE_OPTION,
			__( 'Events per page', 'mime-simple-events-calendar' ),
		...
		add_settings_field(
			EventArchiveSettings::DEFAULT_PERIOD_OPTION,
			__( 'Default period', 'mime-simple-events-calendar' ),
		...
		add_settings_section(
			...
			__( 'Display', 'mime-simple-events-calendar' ),
		...
		add_settings_field(
			'wpse_site_timezone',
			__( 'Site timezone', 'mime-simple-events-calendar' ),
		...
		add_settings_field(
			EventTimezoneDisplaySettings::OPTION,
			__( 'Public event timezone', 'mime-simple-events-calendar' ),
		...
		add_settings_field(
			StructuredDataSettings::OPTION,
			__( 'Event structured data', 'mime-simple-events-calendar' ),
		...
		add_settings_section(
			...
			__( 'Data ownership', 'mime-simple-events-calendar' ),
		...
		add_settings_field(
			UninstallSettings::OPTION,
			__( 'Delete plugin data', 'mime-simple-events-calendar' ),
	...
	public function render_page(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You are not allowed to manage event settings.', 'mime-simple-events-calendar' ) );
		}
		?>
		<div class="wrap">
	...
	public function render_archive_section(): void {
		echo '<p>'
			. esc_html__( 'Configure the native event archive. Visitors can still switch between upcoming, past and all events with the archive filter.', 'mime-simple-events-calendar' )
			. '</p>';
	}
	...
	public function render_archive_slug_field(): void {
		...
		<p class="description"><?php esc_html_e( 'One URL segment without slashes. This also changes individual event URLs; existing links are not redirected. Saving a changed slug rebuilds WordPress permalink rules once.', 'mime-simple-events-calendar' ); ?></p>
		<?php
	}
	...
	public function render_archive_per_page_field(): void {
		...
		<p class="description"><?php esc_html_e( 'Between 1 and 50 public events per archive page.', 'mime-simple-events-calendar' ); ?></p>
		<?php
	}
	...
	public function render_archive_period_field(): void {
		...
		<select id="<?php echo $name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped immediately above. ?>" name="<?php echo $name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped immediately above. ?>">
			<option value="upcoming" <?php selected( $period->value, EventPeriod::UPCOMING->value ); ?>><?php esc_html_e( 'Upcoming and active events', 'mime-simple-events-calendar' ); ?></option>
			<option value="all" <?php selected( $period->value, EventPeriod::ALL->value ); ?>><?php esc_html_e( 'All events, including past events', 'mime-simple-events-calendar' ); ?></option>
		</select>
		<?php
	}
	...
	public function render_archive_conflict_notice(): void {
		...
		$message = sprintf(
			...
			__( 'The event archive and an existing WordPress page both use /%s/. Keep this slug to use the native event archive at that address, or change it below so the page can use the address.', 'mime-simple-events-calendar' ),
			$slug
		);
	...
	public function render_section(): void {
		echo '<p>'
			. esc_html__( 'Control plugin output that may overlap with your theme or SEO plugin.', 'mime-simple-events-calendar' )
			. '</p>';
	}
	...
	public function render_site_timezone_field(): void {
		...
		<p class="description"><?php esc_html_e( 'New events capture this timezone. Existing events keep the timezone saved with them when the WordPress setting changes.', 'mime-simple-events-calendar' ); ?></p>
		<?php if ( $fixed_offset ) : ?>
			<p class="description"><?php esc_html_e( 'This fixed UTC offset does not adjust for daylight saving time.', 'mime-simple-events-calendar' ); ?></p>
		<?php endif; ?>
		<?php if ( current_user_can( 'manage_options' ) ) : ?>
			<p><a href="<?php echo esc_url( admin_url( 'options-general.php' ) ); ?>"><?php esc_html_e( 'Change the site timezone in WordPress General Settings', 'mime-simple-events-calendar' ); ?></a></p>
		<?php endif; ?>
		<?php
	}
	...
	public function render_data_section(): void {
		echo '<p>'
			. esc_html__( 'Event content is preserved when the plugin is deactivated or deleted unless you explicitly choose otherwise below.', 'mime-simple-events-calendar' )
			. '</p>';
	}
	...
	public function render_structured_data_field(): void {
		...
		<label>
			<input id="<?php echo $name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped immediately above. ?>" type="checkbox" name="<?php echo $name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped immediately above. ?>" value="1" <?php echo checked( $enabled, true, false ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Core checked() returns a fixed HTML attribute. ?>>
			<?php esc_html_e( 'Output JSON-LD on individual public event pages.', 'mime-simple-events-calendar' ); ?>
		</label>
		<p class="description">
			<?php esc_html_e( 'Disable this when another SEO plugin already outputs Event structured data.', 'mime-simple-events-calendar' ); ?>
		</p>
		<?php
	}
	...
	public function render_uninstall_field(): void {
		...
		<label>
			<input id="<?php echo $name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped immediately above. ?>" type="checkbox" name="<?php echo $name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped immediately above. ?>" value="1" <?php echo checked( $enabled, true, false ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Core checked() returns a fixed HTML attribute. ?>>
			<?php esc_html_e( 'Permanently delete events, event categories, event tags, plugin settings and event capabilities when the plugin is deleted.', 'mime-simple-events-calendar' ); ?>
		</label>
		<p class="description">
			<strong><?php esc_html_e( 'This cannot be undone.', 'mime-simple-events-calendar' ); ?></strong>
			<?php esc_html_e( ' Uploaded media is retained because it may be used elsewhere. Deactivation never deletes data.', 'mime-simple-events-calendar' ); ?>
		</p>
		<?php
	}
	...
	private function render_maintenance_tools(): void {
		...
		<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
			...
			<?php submit_button( __( 'Repair event capabilities', 'mime-simple-events-calendar' ), 'secondary', 'submit', false ); ?>
		</form>
		<p class="description"><?php esc_html_e( 'Restores the documented event permissions for administrators and editors without changing other role permissions.', 'mime-simple-events-calendar' ); ?></p>
		...
		<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
			...
			submit_button(
				'reindex_progress' === $state['status']
					? __( 'Continue rebuilding date indexes', 'mime-simple-events-calendar' )
					: __( 'Rebuild event date indexes', 'mime-simple-events-calendar' ),
				'secondary',
				'submit',
				false
			);
		...
		<p class="description"><?php esc_html_e( 'Recalculates only derived UTC indexes from validated local event dates, in batches of 50. Invalid events are skipped for manual review.', 'mime-simple-events-calendar' ); ?></p>
		<?php
	}
	...
	private function render_maintenance_notice( array $state ): void {
		if ( 'capabilities_repaired' === $state['status'] ) {
			echo '<div class="notice notice-success inline"><p>'
				. esc_html__( 'Event capabilities were restored for administrators and editors.', 'mime-simple-events-calendar' )
				. '</p></div>';
			return;
		}
		...
		$message = sprintf(
			...
			__( 'Date index maintenance inspected %1$d events: %2$d changed, %3$d skipped as invalid and %4$d write failures.', 'mime-simple-events-calendar' ),
			$state['processed'],
			$state['changed'],
			$state['skipped'],
			$state['failed']
