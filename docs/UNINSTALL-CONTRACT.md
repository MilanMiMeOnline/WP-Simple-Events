...
Deactivating MiMe Simple Events and Calendar never deletes anything. Deleting the plugin also keeps events, event terms, options and event capabilities unless an administrator first enables **Events → Settings → Delete plugin data**.
...
- plugin-owned schema, archive-routing, structured-data, timezone-display, native-calendar-action, pending-rewrite and uninstall-preference options;
