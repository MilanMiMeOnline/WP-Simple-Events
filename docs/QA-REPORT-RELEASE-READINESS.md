...
**Date:** 2026-07-17\
**Candidate:** MiMe Simple Events and Calendar 0.1.0\
**PHP smoke runtime:** 8.3\
**Archive SHA-256:** `fee5f9bb42f1f23ddb98ba45395bdb705fdf4fee1432be42be2ccf0947f1fa05`

## Delivered controls

- A production allowlist builds `.release/mime-simple-events-calendar` instead of archiving the working tree.
...
- The required `/languages/mime-simple-events-calendar.pot` is generated deterministically and checked for freshness in CI.
...
1. The official `wordpress/plugin-check-action@v1` job is configured correctly against `.release/mime-simple-events-calendar`, but its result must still be observed green in GitHub Actions for the release commit. Local Docker is unavailable, and Plugin Check 2.0.0 was too heavy to run reliably inside the WebAssembly Playground; no flaky local substitute was retained.
