...
MiMe Simple Events and Calendar stays small by favouring native WordPress concepts: a custom post type, taxonomies, metadata, capabilities, REST schemas, templates and bounded `WP_Query` objects. Abstractions must remove real duplication or isolate a genuine boundary; speculative frameworks are out of scope.
