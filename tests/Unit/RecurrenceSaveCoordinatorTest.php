...
final class RecurrenceSaveCoordinatorTest extends TestCase {
	...
	public function test_changed_aggregate_is_stored_then_projected(): void {
		...
		$result    = $this->coordinator( new FakeRecurrenceAggregateStore(), $projector )
			->save( 42, $this->aggregate(), $this->window() );
	...
	public function test_unchanged_clean_aggregate_skips_projection(): void {
		...
		$result = $this->coordinator( $store, $projector )->save( 42, $this->aggregate(), $this->window() );
	...
	private function coordinator(
		...
		return new RecurrenceSaveCoordinator( new RecurrenceAggregatePersistence( $store ), $projector );
	}
	...
	/**
	 * Return one bounded projection window.
	 */
	private function window(): RecurrenceGenerationWindow {
		return RecurrenceGenerationWindow::between( '2027-01-04', '2027-02-04' );
	}
}
