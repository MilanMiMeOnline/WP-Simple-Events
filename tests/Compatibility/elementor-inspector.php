...
function wpse_run_elementor_compat_inspection(): void {
	...
	wpse_compat_require( did_action( 'wpse_loaded' ) > 0, 'MiMe Simple Events and Calendar did not boot.' );
	...
	foreach ( $names as $name ) {
		...
		wpse_compat_require( in_array( 'mime-simple-events-calendar', $widgets[ $name ]->get_categories(), true ), 'Wrong widget category: ' . $name );
	...
	WP_CLI::success( 'MiMe Simple Events and Calendar Elementor compatibility contract passed.' );
}
